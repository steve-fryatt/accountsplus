...
ZIPFILE := accountsplus$(RELEASE).zip
SRCZIPFILE := accountsplus$(RELEASE)src.zip
BUZIPFILE := accountsplus$(shell date "+%Y%m%d").zip
...
BINDHELPFLAGS := -f -r -v
TOKFLAGS := -verbose -increment 1 -crunch EIrW -warn pV -swi -swis $(GCCSDK_INSTALL_CROSSBIN)/../arm-unknown-riscos/include/swis.h -swis $(GCCSDK_INSTALL_ENV)/include/TokSWIs.h
...
SRCS := AccountsPlus.bbt HelpLib.bbt
